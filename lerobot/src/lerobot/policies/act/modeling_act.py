...
class ACTPolicy(PreTrainedPolicy):
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        # ============== [TORQUE EXTENSION] ==============
        # Slice action to skip first N dimensions (e.g., base velocity)
        if self.config.action_dim_offset > 0:
            batch = dict(batch)  # shallow copy
            batch[ACTION] = batch[ACTION][..., self.config.action_dim_offset:]
        # ================================================
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.use_vae:
            ...
            action_dim = self.config.action_feature.shape[0] - config.action_dim_offset
            # ================================================
        ...
        # ============== [TORQUE EXTENSION] ==============
        # Torque projection layer (always created, input may be zeros if use_torque=False)
        if self.config.robot_state_feature:
            self.encoder_torque_input_proj = nn.Linear(
                self.config.robot_state_feature.shape[0], config.dim_model  # same dim as state (14D)
            )
        # ================================================
        if self.config.image_features:
        ...
        # ============== [TORQUE EXTENSION] ==============
        if self.config.robot_state_feature:
            n_1d_tokens += 1  # for the torque token (always present)
        # ================================================
        if self.config.env_state_feature:
            n_1d_tokens += 1
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if self.config.robot_state_feature:
            if self.config.use_torque and "observation.effort" in batch:
                # Use real torque data (already normalized by dataloader)
                torque = batch["observation.effort"]
            else:
                # Use zeros when use_torque=False or effort not in batch
                torque = torch.zeros_like(batch[OBS_STATE])
            encoder_in_tokens.append(self.encoder_torque_input_proj(torque))
        # ================================================
